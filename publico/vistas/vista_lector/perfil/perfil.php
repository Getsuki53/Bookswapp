...
if (isset($_GET['accion']) && $_GET['accion'] === 'imagen')
{
    ...
    if ($tipo === 'libro') // Consultar la imagen del libro en la base de datos
        $query = "SELECT Lib_imagen FROM libro WHERE Lib_cod = ?";
    elseif ($tipo === 'usuario') // Consultar la imagen del usuario en la base de datos
        $query = "SELECT Lec_img FROM lector WHERE Lec_mail = ?";
    else // Tipo no válido, mostrar una imagen por defecto
    {
